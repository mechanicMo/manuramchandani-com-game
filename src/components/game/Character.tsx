...
const MOVE_SPEED    = 4;
const DESCEND_BASE  = 3;   // auto-descent speed in descent phase
const DESCEND_TUCK  = 6;   // speed when holding ↓ (tuck)
...
export const Character = ({ onPositionChange, holds = [], gamePhase = "ascent" }: Props) => {
  ...
  useFrame((_state, delta) => {
    ...
    if (gamePhase === "descent") {
      // --- DESCENT: auto-descend, carve left/right, tuck with ↓ ---
      const descentSpeed = downPressed ? DESCEND_TUCK : DESCEND_BASE;
      ...
      // Face forward for snowboard
      group.current.rotation.y = 0;

      // Snowboard animation
      if (actions) {
        if (velocity.x > 0.1)       crossFade(actions, "snowboard_right");
        else if (velocity.x < -0.1) crossFade(actions, "snowboard_left");
        else                         crossFade(actions, "snowboard_idle");
      }
    } else {
      // --- ASCENT / SUMMIT: hold-jump + free climb ---
      ...
      if (actions && mixer) {
        ...
        else if (movingY)             crossFade(actions, velocity.y > 0 ? "climb_up" : "climb_down");
